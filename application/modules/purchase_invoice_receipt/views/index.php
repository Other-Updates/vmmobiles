...
<div class="mainpanel">
		...
		$.ajax(
	   {
		url:BASE_URL+"purchase_invoice_receipt/checking_invoice_checkno",
		type:'POST',
		data:{ value1 : checkno},
		success:function(result)
		{
		   $("#ddduperror").html(result);
		...
		$('#cor_no').live('keyup',function(){
	   	...
	   	$('#customer').live('change',function(){
			for_loading(); 
			$.ajax({
				  url:BASE_URL+"purchase_invoice_receipt/get_all_pending_invoice",
				  type:'GET',
				  data:{
					  c_id:$(this).val()
					   },
			...
			$.ajax({
				  url:BASE_URL+"purchase_invoice_receipt/get_invoice_view",
				  type:'GET',
				  data:{
					  c_id:$(this).val()
					   },
		...
		$('.so_id').live('click',function(){
			...
			for_loading(); 
			$.ajax({
				  url:BASE_URL+"purchase_invoice_receipt/get_inv",
				  type:'GET',
				  data:{
					  	inv_id:s_arr,
					  	c_id:$('#customer').val()

...
//        }
    ...
    public function get_product($atten_inputs) {
        ...
        $this->db->select('erp_product.id,model_no,product_name,product_description,product_image,type,cost_price,cgst,sgst,igst,discount,category_id,brand_id,unit,erp_stock.quantity,erp_product.hsn_sac,erp_product.cost_price_without_gst');
    ...
    public function get_all_po_for_report($search = NULL) {
        ...
        if ($search != NULL && $search != '') {
            ...
            if ($search_data[3]->from != '') {
                ...
                $this->db->where($this->erp_po . '.created_date >=', $search_data[3]->from);
            }

            if ($search_data[4]->to != '') {

                $this->db->where($this->erp_po . '.created_date <=', $search_data[4]->to);
            }
        }

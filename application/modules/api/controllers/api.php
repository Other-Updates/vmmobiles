...
class Api extends MX_Controller {
    ...
    public function signup() {
        ...
        if (!empty($json_input)) {
            ...
            if (!empty($data['firms']) && !empty($data['password']) && !empty($data['username']) && !empty($data['name']) && !empty($data['nick_name']) && !empty($data['address']) && !empty($data['mobile']) && !empty($data['role']) && !empty($data['email'])) {
                ...
                $input_data = array(
                    'name' => $name, 'address' => $address, 'nick_name' => $nick_name,
    ...
    public function api_get_all_invoice_id() {
        ...
        $data = json_decode($json_input, TRUE);
        $output = array();
        $list = $this->api_model->get_customer_based_datatables($data);
        $i = 0;
        $data = array();
        foreach ($list as $val) {
            $data[$i]['id'] = $val['id'];
    ...
    public function pdf_monthly_users_details() {
        ...
        if (!empty($json_input)) {
            ...
            /* $json_input = file_get_contents('php://input', TRUE);
              ...
              $i++; */
            $data["monthly_users_details"] = $datas;
    ...
    public function get_purchase_order_by_id() {
        $json_input = file_get_contents('php://input', TRUE); // JSON Input
        $response = array();
        if (!empty($json_input)) {
            $data = json_decode($json_input, TRUE);
            $po_id = $data['po_id'];
            $response['po'] = $this->api_model->get_all_po_by_id($po_id);
            $response['po_details'] = $this->purchase_order_model->get_all_po_details_by_id($po_id);
            $response["category"] = $this->api_model->get_category_by_frim_id($response['po'][0]['firm_id']);
            //print_r($data);
            if (!empty($response)) {
                $output = array('status' => 'Success', 'message' => 'Purchase Order Details', 'data' => $response);
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Data Not Found');
                echo json_encode($output);
    ...
    public function update_purchase_order_by_id() {
        $json_input = file_get_contents('php://input', TRUE); // JSON Input
        if (!empty($json_input)) {
            $input = json_decode($json_input, TRUE);
            //  $output = array('status' => 'Success', 'message' => $input);
            ...
            $po['pr_no'] = $input['pr_no'];
            $po['po_no'] = $input['po_no'];
            $po['delivery_status'] = $input['delivery_status'];
            $po['pr_status'] = $input['pr_status'];
            $po['created_date'] = $input['created_date'];
            $po['po_type'] = $input['po_type'];
            $po['total_qty'] = $input['total']['total_qty'];
            $po['subtotal_qty'] = $input['total']['sub_total'];
            $po['tax_label'] = $input['total']['tax_label'];
            $po['net_total'] = $input['total']['net_total'];
            $po['remarks'] = $input['total']['remarks'];
...
//            }
            ...
            if ($input['delivery_status'] == 'delivered') {
                $po['delivery_qty'] = $input['total']['total_qty'];
            }
            $po['created_date'] = date('Y-m-d', strtotime($input['created_date']));
            $po['po_no'] = $input['pr_no'];
            // $insert_id = $this->purchase_order_model->insert_po($input['po']);


            $update = $this->purchase_order_model->update_po($po, $po_id);
            
            $this->purchase_order_model->delete_po_deteils_by_id($po_id);

            if ($input['pr_status'] == 'approved') {
                $po['po_id'] = $po_id;
                $this->purchase_return_model->delete_pr($po_id);
                $pr_id = $this->purchase_return_model->insert_pr($po);
            }
            ...
            if (isset($product) && !empty($product)) {
                $insert_arr = array();
                $this->purchase_order_model->delete_ime_by_poid($po_id);
            ...
            if ($update == true && !empty($product)) {
                $output = array('status' => 'Success', 'message' => 'Purchase Order Successfully Updated..');
                // $output = array('status' => 'Success', 'message' => $input);
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Purchase Order Not updated..');
                echo json_encode($output);
            }
        } else {
            $output = array('status' => 'error', 'message' => 'Enter Purchase Order Details');
    ...
    public function get_sales_invoice_by_id() {
        ...
        if (!empty($json_input)) {
            ...
            $response["invoice"] = $quotation = $this->project_cost_model->get_all_invoice_by_id($inv_id);

            $response['quotation_details'] = $this->project_cost_model->get_all_invoice_details_by_id($inv_id);
            //print_r($data);
            if (!empty($response)) {
                $output = array('status' => 'Success', 'message' => 'Sales Invoice Details', 'data' => $response);
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Data Not Found');
                echo json_encode($output);
            ...
            $output = array('status' => 'error', 'message' => 'Enter Invoice ID');
            echo json_encode($output);
        }
        exit;
    }

    public function update_sales_invoice_by_id() {
        $json_input = file_get_contents('php://input', TRUE); // JSON Input
        // echo "<pre>";print_r($json_input);exit;
        if (!empty($json_input)) {
            $input = json_decode($json_input, TRUE);
            ...
            $q_id = $input['q_id'];
            $inv_id = $input['inv_id'];
            $sales_inv_id = $input['sales_inv_id'];
            $user_info = $this->api_model->get_user_info_by_id($input['user_id']);
            ...
            $quotation['remarks'] = $input['total']['remarks'];
            $quotation['delivery_schedule'] = date('Y-m-d');
            $quotation['notification_date'] = date('Y-m-d');
            $quotation['mode_of_payment'] = ($input['total']['mode_payment']) ? $input['total']['mode_payment'] : '-';
            $quotation['validity'] = '-';
            $quotation['customer'] = $customer_id;
            $quotation['estatus'] = 2;
            $quotation['created_by'] = $user_info[0]['id'];
            ...
            if (isset($product) && !empty($product)) {
                $insert_id = $this->api_model->update_quotation($quotation, $q_id);
            }
...
//            }
            ...
            $insert_id1 = $this->api_model->edit_invoice($invoice, $inv_id);
...
//            $sr['created_by'] = $user_info[0]['id'];
            ...
            $delete_id = $this->api_model->delete_invoice_deteils_by_id($sales_inv_id);
            //echo $inv_id;
            //exit;
            ...
            if (isset($insert_id1) && !empty($insert_id1)) {
                ...
                $q_id = $this->sales_receipt_model->insert_receipt_bill($insert_data);

                $q_id++;

                $inc['type'] = 'rp_code';

                $inc['value'] = 'RECQ000' . $q_id;
            ...
            if (!empty($insert_id) && !empty($insert_id1)) {
                $output = array('status' => 'Success', 'message' => 'Invoice Successfully Updated..');
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Invoice Not updated..');
                echo json_encode($output);
            }
        } else {
            $output = array('status' => 'error', 'message' => 'Enter Invoice Details');

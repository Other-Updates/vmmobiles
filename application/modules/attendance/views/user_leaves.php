...
<div class="contentinner">
    <h4 class="widgettitle">Apply or Modify User Leaves </h4>           
					...
					if(isset($users) && !empty($users))
					{
						...
						?>	
							<tr class="<?=$class?>">
                              ...
                              <?php
							  $v=0; 
							  if(in_array("leave:view_user_leaves",$user_role)){
							  	 $url = $this->config->item('base_url')."attendance/leave/view_user_leaves/".$user["id"];
								 if($class!="")
							  	 $url = $this->config->item('base_url')."attendance/leave/view_user_leaves/".$user["id"]."?us=1";
							  ?>
							  ...
							  ?>
                              <a href="<?=$this->config->item('base_url')."attendance/leave/apply_leave/".$user["id"]?>"  title="Apply leave" class="btn btn-danger border4">                              Apply Leave / OD</a> 
                              </td>
                              <?php
							  $v=1;
							  }

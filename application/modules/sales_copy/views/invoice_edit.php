...
//                                var tab_save = parseInt(tab_model) + parseInt(1);
    ...
    function ime_code_scan() {
        ...
        if (cust_id == "") {
            ...
            toastr.clear();
            toastr.error("IMIE code field is required", 'Warning Message.!');
            return false;
        } else if ((!($.isNumeric(bar_code))) && bar_code.length < 15) {
            toastr.clear();
            toastr.error("Invalid IMIE Code", 'Warning Message.!');
            return false;
            ...
            $('#app_table .ime_code_id').each(function () {
                ...
                if (hidden_ime != "null") {
                    if (jQuery.inArray(barcode, hidden_ime) != '-1') {
                        toastr.clear();
                        toastr.error("IMIE Code Alreday Added", 'Warning Message.!');
                        ime++;
                        $(this).closest("td").find('.ime_code_error').text("IMIE Cdoe Alreday Added");
